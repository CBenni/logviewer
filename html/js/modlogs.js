logviewerApp.controller("ModLogController", function($scope, $http, $stateParams, $rootScope, $sce, logviewerSocket, $mdDialog, $timeout, $q){
	...
	$scope.loadStatus = 0;
	$scope.filters = {commands: {}, include: "", exclude: ""};
	$scope.commands = ["timeout","ban","unban","untimeout","slow","slowoff","subscribers","subscribersoff","followers","followersoff","emoteonly","emoteonlyoff","clear","host","unhost","commercial","mod","unmod"];
	for(var i=0;i<$scope.commands.length;++i) {
		$scope.filters.commands[$scope.commands[i]] = "1";
	}
	$scope.filters.commands.timeout = undefined;
	...
	// virtualRepeat model
	var infiniteScroller = function (filters) {
	...
	$scope.setFilters() {
		$scope.allUsers = new infiniteScroller($scope.filters);
	}
});